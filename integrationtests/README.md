# Automated Tests with WebDriveIO
...
First, you must build the application from the application root directory using the following command:

```sh
SOA_ROOT=localhost:8081 yarn run build
```

_Note: The tests assume a SOA root of https://localhost:8081 which is the URL for the WireMock service._

Then to run your tests, just call the [WDIO runner](http://webdriver.io/guide/testrunner/gettingstarted.html) from the `integrationtests` directory:

```sh
$ yarn run wdio
```

_Note: This will also start a wiremock service on port 8081_

# Running Wiremock

Although `yarn run wdio` will run the wiremock service automatically for the duration of the test, we can start wiremock manually as a standalone service by running the following comand within the `integrationtests` directory.

```sh
$ yarn run wiremock:standalone
```

This is particularly useful when developing wiremock mappings.

_Note: You should not run this when running the tests as webdriverio starts the wiremock service itself._
