...
import waitForPath from '../support/action/waitForPath';

Given(/^I open the Audi (url|site) "([^"]*)?"$/, openWebsite);

Given(/^that the path (becomes|will contain) "([^"]*)"$/, waitForPath);

Given(/^the page url is( not)* "([^"]*)?"$/, checkUrl);

Given(/^I am( not)* on the "([^"]*)?" page$/, checkURLPath);
