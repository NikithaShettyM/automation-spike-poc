...
export default (obsolete, falseCase) => {
  ...
  const windowHandles = browser.getWindowHandles().length;

  if (falseCase) {
    expect(windowHandles).toBe(1, 'A new window should not have been opened');
  } else {
    expect(windowHandles).not.toBe(1, 'A new window has been opened');
  }
};
