...
export default (isCSS, attrName, selector, falseCase, expectedValue) => {
   ...
   * @type {Mixed}
   */
  let attributeValue = browser.$app(getSelector(selector))[command](attrName);
  ...
  if (falseCase) {
    expect(attributeValue).not.toBe(
      expectedValue,
      `${attrType}: ${attrName} of element "${selector}" should not ` +
        `contain "${attributeValue}"`
    );
  } else {
    expect(attributeValue).toBe(
      expectedValue,
      `${attrType}: ${attrName} of element "${selector}" should contain ` +
